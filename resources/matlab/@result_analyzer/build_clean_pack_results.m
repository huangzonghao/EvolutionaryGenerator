...
% The ideal use case is to process and pack a newly finished training group
    ...
    for i = 1 : num_results
        waitbar(double(i - 1) / double(num_results), wb, sprintf("Processing %d / %d - building", i, num_results));
        ...
        % clean
        waitbar(double(i - 1 + 0.3) / double(num_results), wb, sprintf("Processing %d / %d - cleaning", i, num_results));
        app.clean_result(result);

        % export
        % waitbar(double(i - 1 + 0.5) / double(num_results), wb, sprintf("Processing %d / %d - exporting", i, num_results));
        % wb.Children.Title.String = sprintf("Processing %d / %d - exporting", i, num_results);
        % app.export_result(result, export_dir);

        % pack
        waitbar(double(i - 1 + 0.8) / double(num_results), wb, sprintf("Processing %d / %d - packing", i, num_results));
