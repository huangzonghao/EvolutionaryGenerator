function plot_result_stat(app)
    ...
    % Fitness Plot
    sp{1} = subplot(num_subplots, 1, 1, 'NextPlot', 'add');
    plot(start_gen : end_gen, stat.archive_fits(start_gen + 1 : end_gen + 1), 'LineWidth', 2, 'DisplayName', 'Archive Mean');
    ...
    sp{2} = subplot(num_subplots, 1, 2, 'NextPlot', 'add');
    change_to_prev_gen = [];
    ...
    plot(start_gen + 1 : end_gen, change_to_prev_gen);
    ...
    sp{3} = subplot(num_subplots, 1, 3, 'NextPlot', 'add');
    change_to_prev_gen = [];
    for i = start_gen + 2 : end_gen + 1 % (start_gen + 1) is the data index for start_gen
    ...
    % Coverage Plot
    sp{4} = subplot(num_subplots, 1, 4, 'NextPlot', 'add');
    plot(start_gen : end_gen, stat.coverage(start_gen + 1 : end_gen + 1), 'b');
