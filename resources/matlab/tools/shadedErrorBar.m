% Adopted from https://github.com/raacampbell/shadedErrorBar
function varargout = shadedErrorBar(ax, x, y, errBar, varargin)
% generate continuous error bar area around a line plot
%
% function H = shadedErrorBar(ax, x, y, errBar, ...)
%
% Purpose
...
function H = makePlot(ax, x, y, errBar, lineProps, transparent, patchSaturation, displayname)
    ...
    if isdatetime(x)
        H.patch = patch(ax, datenum(xP), yP, 1);
    else
        H.patch = patch(ax, xP, yP, 1);
    end
    ...
    H.edge(1) = plot(ax, x, lE, '-');
    H.edge(2) = plot(ax, x, uE, '-');
